...
// For incremental solving support
struct incremental_frame {
  size_t stored_clauses_size;
  unsigned next_var;
...
struct kissat {
  ...
  unsigned level;
  frames frames;  // Original decision level frames
  ...
  unsigned next_var;           // Next variable ID to be allocated
  bool needs_rebuild;          // Whether solver state needs to be rebuilt
  unsigneds stored_clauses;    // Store all clauses for incremental solving
...
// Internal functions for incremental solving
void kissat_internal_add(kissat *solver, int lit);
void kissat_clear(kissat *solver);
