...
void kissat_release (kissat *solver) {
  ...
  // Release incremental solving stacks
  RELEASE_STACK(solver->stored_clauses);
  RELEASE_STACK(solver->assumptions);
  RELEASE_STACK(solver->inc_frames);
...
#endif
  if (lit) {
    kissat_require_valid_external_internal (lit);
#if !defined(NDEBUG) || !defined(NPROOFS) || defined(LOGGING)
    if (checking || logging || proving)
      PUSH_STACK (solver->original, lit);
#endif
    unsigned ulit = kissat_import_literal (solver, lit);

    const mark mark = MARK (ulit);
    if (!mark) {
      const value value = kissat_fixed (solver, ulit);
      if (value > 0) {
        if (!solver->clause_satisfied) {
          LOG ("adding root level satisfied literal %u(%d)@0=1", ulit,
               lit);
          solver->clause_satisfied = true;
        }
      } else if (value < 0) {
        LOG ("adding root level falsified literal %u(%d)@0=-1", ulit, lit);
        ...
        PUSH_STACK (solver->clause, ulit);
      }
    } else if (mark < 0) {
      assert (mark < 0);
      if (!solver->clause_trivial) {
        LOG ("adding dual literal %u(%d) and %u(%d)", NOT (ulit), -lit,
             ulit, lit);
        solver->clause_trivial = true;
      }
    } else {
      assert (mark > 0);
      LOG ("adding duplicated literal %u(%d)", ulit, lit);
      if (!solver->clause_shrink) {
        solver->clause_shrink = true;
        LOG ("thus original clause needs shrinking");
      }
    ...
    // Store clause for incremental solving
    if (lit) {
      PUSH_STACK(solver->stored_clauses, (unsigned)lit);
    }
  } else {
#if !defined(NDEBUG) || !defined(NPROOFS) || defined(LOGGING)
...
static void kissat_clear_internal_state(kissat *solver) {
  solver->inconsistent = false;
  CLEAR_STACK(solver->clause);
  // Reset internal state but keep stored clauses and assumptions
  solver->next_var = 1;
  solver->needs_rebuild = false;
}
...
int kissat_solve (kissat *solver) {
  ...
  if (solver->needs_rebuild) {
    rebuild_solver_if_needed(solver);
  }
...
void kissat_pop(kissat *solver) {
  assert(!EMPTY_STACK(solver->inc_frames));
